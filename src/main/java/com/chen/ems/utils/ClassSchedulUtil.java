...
public class ClassSchedulUtil {
    ...
    public static String cutGene(String aim, String source) {
        String[] genes = source.split(",");
        switch (aim) {
            case Constants.IS_FIX:
                return genes[0];
            case Constants.COLLEGE_NO:
                return genes[1];
            case Constants.CLASS_NO:
                return genes[2];
            case Constants.TEACHER_NO:
                return genes[3];
            case Constants.COURSE_NO:
                return genes[4];
            case Constants.COURSE_ATTR:
                return genes[5];
            case Constants.CLASS_TIME:
                return genes[6];
            case Constants.CLASSROOM_NO:
                return genes[7];
            default:
                return "";
        }
    ...
    private static Boolean isTimeRepe(String time, String gene, List<String> geneList) {
        //获得班级编号
        String classNo = cutGene(Constants.CLASS_NO, gene);
        for (String str : geneList) {
            //判断班级编号是否相等
            if (classNo.equals(cutGene(Constants.CLASS_NO, str))) {
                //班级编号相等的则判断时间是否有重复，没有返回true
                String classTime = cutGene(Constants.CLASS_TIME, str);
...
//    计算适应度值
    public static double alculateExpectedValue(List<String> individualList) {
        ...
        for (String gene : individualList) {
            String courseAttr = cutGene(Constants.COURSE_ATTR, gene);//获得属性
            String classTime = cutGene(Constants.CLASS_TIME, gene);//获得该课程的开课时间
            if (courseAttr.equals(Constants.PROFESSIONAL_COURSE)) {
                F1 = F1 + calculateProfessExpect(classTime);
            } else if (courseAttr.equals(Constants.PUBLIC_ELECTIVE_COURSE)) {
                F2 = F2 + calculateElectiveExpect(classTime);
            } else if (courseAttr.equals(Constants.BASIC_COURSE)) {
